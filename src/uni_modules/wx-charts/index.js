...
function getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
  ...
  data.forEach(function (item, index) {
    if (item === null) {
      ...
      height *= process
      point.y =
        opts.height - config.xAxisHeight - config.legendHeight - Math.round(height) - config.padding
      points.push(point)
    }
  })
...
function drawToolTip(textList, offset, opts, config, context) {
  ...
  var toolTipWidth =
    legendWidth + legendMarginRight + 4 * config.toolTipPadding + Math.max.apply(null, textWidth)
  ...
  series.forEach(function (eachSeries, seriesIndex) {
    ...
    points.forEach(function (item, index) {
      if (item !== null) {
        ...
        var height =
          opts.height - item.y - config.padding - config.xAxisHeight - config.legendHeight
...
function drawXAxis(categories, opts, config, context) {
  ...
  var maxXAxisListLength = Math.min(
    categories.length,
...
var Charts = function Charts(opts) {
  ...
  config$$1.yAxisTitleWidth =
    opts.yAxis.disabled !== true && opts.yAxis.title ? config$$1.yAxisTitleWidth : 0
