!(function (e, n) {
  ...
  const d = {
    ...
    navigateBack() {
      const e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {}
      const n = e.delta
      r('navigateBack', { delta: Number.parseInt(n) || 1 })
    },
    switchTab() {
      const e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {}
      const n = e.url
  ...
  const s = /uni-app/i.test(navigator.userAgent)
  const w = /Html5Plus/i.test(navigator.userAgent)
  const u = /complete|loaded|interactive/
  const g =
    window.my &&
    navigator.userAgent.includes(['t', 'n', 'e', 'i', 'l', 'C', 'y', 'a', 'p', 'i', 'l', 'A'].reverse().join(''))
  ...
  S || (S = {})
  const P = typeof uni != 'undefined' ? uni : {}
  if (!P.navigateTo) {
